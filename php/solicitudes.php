...
$query = $mysqli->query("SELECT p.nombre_producto, p.foto, m.nombre_marca, mo.nombre_modelo, s.fecha_solicitud, s.hora_solicitud, e.nombre_estsol
                        ...
                        WHERE p.id_marca = m.id_marca and e.id_estsol = s.id_estsol and p.id_modelo = mo.id_modelo and s.id_cotizacion = (SELECT c.id_cotizacion from cotizacion_credito c where c.id_producto = p.id_producto limit 1) and s.id_cliente = $id order by s.fecha_solicitud desc, s.hora_solicitud desc");
